...
export async function generateEpiRecordPdf(worker: Worker, epiAssignments: EpiAssignment[], mode: 'download' | 'preview' = 'download') {
  ...
  if (settings?.logo_url) {
    try {
      doc.addImage(settings.logo_url, 'PNG', 15, 12, 40, 16);
    } catch (e) {
      doc.setFontSize(18);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(230, 81, 0); // Orange
      doc.text(settings?.company_name || 'EngenharQ', 15, 23);
    }
  } else {
    doc.setFontSize(18);
    doc.setFont('helvetica', 'bold');
  ...
  for (let i = 0; i < maxRows; i++) {
    ...
    if (epiAssignments && epiAssignments[i]) {
      ...
      const catalog = assignment.catalog || (assignment as any).epi?.catalog;
      
      doc.text('1', 13, currentY + 5);
      
      if (catalog?.name) {
        const prefix = catalog.category?.toLowerCase().includes('uniforme') ? '[UNIF]' : '[EPI]';
        const itemName = `${prefix} ${catalog.name} (Cód: ${catalog?.code || '-'})`;
        let text = itemName;
